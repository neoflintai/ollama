...
// All computation is eager (no deferred graph execution).
type Tensor struct {
	b     *Backend
	name  string
	shape []int
...
func (t *Tensor) MulmatID(ctx ml.Context, t2, ids ml.Tensor) ml.Tensor {
	// Simplified: just do regular matmul
	return matmul(t, t2.(*Tensor))
}
...
func (t *Tensor) View(ctx ml.Context, offset int, shape ...int) ml.Tensor {
	...
	// offset is in bytes for ggml compatibility, convert to float32 index
	elemOffset := offset / 4
...
func (t *Tensor) Repeat(ctx ml.Context, dim, n int) ml.Tensor {
	return repeat(t, dim, n)
}

func (t *Tensor) Repeat4D(ctx ml.Context, dim0, dim1, dim2, dim3 int) ml.Tensor {
	// Simple implementation: repeat along each dimension
	result := t
	if dim0 > 1 {
		result = repeat(result, 0, dim0).(*Tensor)
	}
	if dim1 > 1 {
		result = repeat(result, 1, dim1).(*Tensor)
	}
	if dim2 > 1 {
		result = repeat(result, 2, dim2).(*Tensor)
	}
	if dim3 > 1 {
		result = repeat(result, 3, dim3).(*Tensor)
	}
	return result
}
...
func (t *Tensor) SetInplace(ctx ml.Context, src ml.Tensor, nb1, nb2, nb3, offset int) ml.Tensor {
	// Simplified: copy src data at offset
	out := make([]float32, len(t.data))
	copy(out, t.data)
	s := src.(*Tensor)
	elemOffset := offset / 4
...
func (t *Tensor) Variance(ctx ml.Context) ml.Tensor {
	...
	return newTensor(t.b, []int{1}, []float32{float32(variance)})
}
	...
	for batch := 0; batch < batchSize; batch++ {
		...
		for r := 0; r < rows; r++ {
			for c := 0; c < cols; c++ {
				...
				case 0: // upper + diag
					keep = c >= r
				case 1: // upper (no diag)
					keep = c > r
				case 2: // lower + diag
					keep = c <= r
				case 3: // lower (no diag)
					keep = c < r
				}
				if !keep {
					out[base+r*cols+c] = 0
...
func (t *Tensor) Conv2D(ctx ml.Context, weight ml.Tensor, s0, s1, p0, p1, d0, d1 int) ml.Tensor {
	// Placeholder: return zeros with approximate shape
...
// Following GGML convention: result = a^T * b for 2D tensors.
// Shape convention: tensor shape is [columns, rows] (column-major).
func matmul(a, b *Tensor) *Tensor {
	...
	// GGML matmul: C = A^T * B
	// A shape: [K, M] -> A is M rows x K cols in row-major
	// B shape: [K, N] -> B is N rows x K cols in row-major
	// C shape: [M, N]
	K := a.shape[0]
	M := a.shape[1]
	N := b.shape[1]

	// Handle batched matmul for higher dims
	batchA := 1
...
func softmax(t *Tensor) *Tensor {
	...
	// Apply softmax along first dimension
	dim0 := t.shape[0]
...
func permute(t *Tensor, dims []int) *Tensor {
	...
	for outIdx := 0; outIdx < total; outIdx++ {
		// Decompose outIdx into new coordinates
		remaining := outIdx
		...
		// Map new coordinates back to old using inverse permutation
		srcIdx := 0
		for d := 0; d < ndim; d++ {
			if dims[d] < ndim {
...
func repeat(t *Tensor, dim, n int) ml.Tensor {
	...
	// Simple repeat by concatenating data
	total := 1
	...
	// Calculate stride for the repeat dimension
	innerSize := 1
	for i := 0; i < dim; i++ {
		innerSize *= t.shape[i]
	}
...
func concat(a, b *Tensor, dim int) *Tensor {
	// Simple concatenation along dim 0
	...
	// For higher dims, just concatenate data (simplified)
	out := make([]float32, len(a.data)+len(b.data))
	copy(out, a.data)
	copy(out[len(a.data):], b.data)
	newShape := a.Shape()
...
func rows(t, indices *Tensor) *Tensor {
	if len(t.shape) == 0 {
...
func sliceTensor(t *Tensor, dim, low, high, step int) *Tensor {
	...
	// For dim 0 (innermost), simple strided copy
	if dim == 0 {
...
func stack(tensors []*Tensor, dim int) *Tensor {
	...
	// Simple stack: concat all data, adjust shape
	totalData := 0
...
// Ensure Tensor implements ml.Tensor
