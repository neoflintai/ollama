...
// kind values map to the TensorType constants in fs/gguf/tensor.go.
func dequantize(data []byte, kind uint32, numElements uint64) []float32 {
	...
	default:
		// Fallback: treat as raw F32 or return zeros
...
// Q4_0: blocks of 32 values, each block = 2 bytes scale (fp16) + 16 bytes (32 nibbles)
func dequantQ4_0(data []byte, n uint64) []float32 {
	const blockSize = 32
	const bytesPerBlock = 2 + 16 // fp16 scale + 16 bytes of nibbles
	out := make([]float32, n)
	nblocks := n / blockSize
...
// Q4_1: blocks of 32 values, each block = 2 bytes scale (fp16) + 2 bytes min (fp16) + 16 bytes nibbles
...
// Q5_0: blocks of 32 values, each block = 2 bytes scale (fp16) + 4 bytes high bits + 16 bytes low nibbles
...
// Q8_0: blocks of 32 values, each block = 2 bytes scale (fp16) + 32 bytes (int8)
...
// Q8_1: blocks of 32 values, each block = 4 bytes (fp32 scale) + 4 bytes (fp32 sum) + 32 bytes (int8)
// Actually: 2 bytes scale (fp16) + 2 bytes sum (fp16) + 32 bytes
func dequantQ8_1(data []byte, n uint64) []float32 {
	...
	for block := uint64(0); block < nblocks; block++ {
		...
		scale := float16ToFloat32(binary.LittleEndian.Uint16(data[off:]))
		// sum at off+2 is unused for dequantization
		for j := 0; j < 32; j++ {
...
// K-quant formats: blocks of 256 values

// Q2_K: super-blocks of 256
func dequantQ2K(data []byte, n uint64) []float32 {
	const blockSize = 256
	const bytesPerBlock = 256/16 + 256/4 + 2 + 2 // scales + qs + d + dmin
	out := make([]float32, n)
	nblocks := n / blockSize
...
func dequantQ4K(data []byte, n uint64) []float32 {
	const blockSize = 256
	const bytesPerBlock = 2 + 2 + 12 + 128 // d + dmin + scales + qs
	out := make([]float32, n)
	nblocks := n / blockSize
	...
	for block := uint64(0); block < nblocks; block++ {
		...
		// Decode scales and mins from 12 bytes -> 8 scales + 8 mins (6 bits each)
		var scales [8]uint8
		var mins [8]uint8
		for i := 0; i < 4; i++ {
...
// Q5_K: super-blocks of 256
func dequantQ5K(data []byte, n uint64) []float32 {
	const blockSize = 256
	const bytesPerBlock = 2 + 2 + 12 + 32 + 128 // d + dmin + scales + qh + qs
	out := make([]float32, n)
	nblocks := n / blockSize
...
func dequantQ6K(data []byte, n uint64) []float32 {
	const blockSize = 256
	const bytesPerBlock = 128 + 64 + 16 + 2 // ql + qh + scales + d
	out := make([]float32, n)
	nblocks := n / blockSize
